...
async function runIntegrationTests() {
  ...
  try {
    ...
    const testText =
      'Hello world, this is a test for semantic search embeddings.';
    ...
    console.log(
      `   - Vector magnitude: ${Math.sqrt(result.embedding.reduce((sum, val) => sum + val * val, 0)).toFixed(4)}`,
    );

    // Validate results
    if (result.embedding.length !== 384) {
      throw new Error(
        `Expected 384 dimensions, got ${result.embedding.length}`,
      );
    }
    ...
    batchResults.forEach((emb, i) => {
      console.log(
        `   ${i + 1}. ${emb.embedding.length} dims, ${emb.tokens} tokens`,
      );
    });
    ...
    const semanticResults =
      await vectorService.generateEmbeddings(semanticTexts);
    ...
    console.log(
      `   - "${semanticTexts[0]}" ↔ "${semanticTexts[1]}": ${sim1_2.toFixed(4)} (similar meanings)`,
    );
    console.log(
      `   - "${semanticTexts[0]}" ↔ "${semanticTexts[2]}": ${sim1_3.toFixed(4)} (different topics)`,
    );
    console.log(
      `   - "${semanticTexts[1]}" ↔ "${semanticTexts[2]}": ${sim2_3.toFixed(4)} (different topics)`,
    );

    // Validate semantic understanding
    if (sim1_2 <= sim1_3) {
      console.warn(
        `⚠️  Warning: Similar texts should have higher similarity (${sim1_2.toFixed(4)} vs ${sim1_3.toFixed(4)})`,
      );
    } else {
      console.log(
        `✅ Semantic understanding validated: similar texts have higher similarity`,
      );
    }
    ...
    console.log(
      `   - Total time for ${benchmarkTexts.length} embeddings: ${totalTime}ms`,
    );
    console.log(`   - Average time per embedding: ${avgTime.toFixed(2)}ms`);
    console.log(
      `   - Throughput: ${(1000 / avgTime).toFixed(2)} embeddings/second`,
    );
