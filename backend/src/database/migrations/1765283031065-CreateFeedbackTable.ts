...
export class CreateFeedbackTable1765283031065 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."feedback_type_enum" AS ENUM('bug_report', 'feature_request', 'ai_error', 'categorization_error', 'summary_error', 'entity_error', 'urgency_error', 'general_feedback', 'content_quality', 'performance_issue')`);
        await queryRunner.query(`CREATE TYPE "public"."feedback_priority_enum" AS ENUM('low', 'medium', 'high', 'critical')`);
        await queryRunner.query(`CREATE TYPE "public"."feedback_status_enum" AS ENUM('new', 'acknowledged', 'in_progress', 'resolved', 'closed')`);
        ...
        await queryRunner.query(`ALTER TABLE "feedback" ADD CONSTRAINT "FK_dbbd980d6121bad602ac9468f51" FOREIGN KEY ("dump_id") REFERENCES "dumps"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "feedback" ADD CONSTRAINT "FK_f03c707ffbeab766b829a374381" FOREIGN KEY ("resolved_by") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "feedback" DROP CONSTRAINT "FK_f03c707ffbeab766b829a374381"`);
        await queryRunner.query(`ALTER TABLE "feedback" DROP CONSTRAINT "FK_dbbd980d6121bad602ac9468f51"`);
