...
export class RecentCommand {
  ...
  async execute(user: User, limit: number = 5): Promise<string> {
    try {
      this.logger.log(`Getting recent dumps for user ${user.id}, limit: ${limit}`);

      const recentDumps = await this.dumpService.getRecentByUser(user.id, limit);

      if (recentDumps.length === 0) {
        return '📋 <b>Recent Content</b>\n\n' +
      ...
      for (const dump of recentDumps) {
        ...
        const content = contentText?.length > 50 
      ...
      this.logger.error(`Error getting recent dumps: ${error.message}`, error.stack);
      return '❌ <b>Error</b>\n\n' +
