...
import { FeedbackService, FeedbackType } from '../../../modules/feedback/feedback.service';
...
export class ReportCommand {
  ...
  async execute(user: User, commandText?: string): Promise<string> {
    try {
      ...
      this.logger.log(`User ${user.id} submitting report: ${reportText || 'No specific text'}`);
      ...
      response += '🙏 Thank you for helping us improve! Our team will review your report and take appropriate action.\n\n';
      ...
      return '❌ <b>Report Submission Failed</b>\n\n' +
  ...
  private showReportOptions(): string {
    ...
    response += '• <code>/report The bot is not saving my voice messages</code>\n';
    response += '• <code>/report Wrong category detected for my receipt</code>\n';
  ...
  private determineFeedbackType(reportText: string): FeedbackType {
    ...
    if (text.includes('bug') || text.includes('error') || text.includes('not working') || text.includes('broken')) {
    ...
    if (text.includes('slow') || text.includes('performance') || text.includes('loading')) {
      return FeedbackType.PERFORMANCE_ISSUE;
    }
    
    if (text.includes('ai') || text.includes('analysis') || text.includes('detection')) {
      return FeedbackType.AI_ERROR;
    }
    
    if (text.includes('feature') || text.includes('suggest') || text.includes('would like')) {
