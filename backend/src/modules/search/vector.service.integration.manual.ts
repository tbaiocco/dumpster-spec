...
describe.skip('VectorService - Real Local Model Integration Test', () => {
  ...
  it('should generate real embedding using local model', async () => {
    ...
    try {
      ...
      console.log(
        `- First 5 embedding values: ${result.embedding
          .slice(0, 5)
          .map((v) => v.toFixed(4))
          .join(', ')}`,
      );
  ...
  it('should handle batch embeddings with local model', async () => {
    ...
    try {
      ...
      for (const [index, result] of results.entries()) {
        ...
        console.log(
          `- Text ${index + 1}: ${result.embedding.length} dimensions, ${result.tokens} tokens`,
        );
      }
      ...
      console.log(
        '✅ Batch local embeddings validated - different texts produce different vectors!',
      );
    } catch (error) {
  ...
  it('should demonstrate semantic similarity with local embeddings', async () => {
    ...
    try {
      ...
      const sim1_2 = service.calculateCosineSimilarity(
        results[0].embedding,
        results[1].embedding,
      );
      const sim1_3 = service.calculateCosineSimilarity(
        results[0].embedding,
        results[2].embedding,
      );
      ...
      console.log(
        `- Similarity between "${texts[0]}" and "${texts[1]}": ${sim1_2.toFixed(4)}`,
      );
      console.log(
        `- Similarity between "${texts[0]}" and "${texts[2]}": ${sim1_3.toFixed(4)}`,
      );
      ...
      console.log(
        '✅ Semantic similarity validation passed - local model understands meaning!',
      );
    } catch (error) {
