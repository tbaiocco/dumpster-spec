...
export class RankingService {
  ...
  async rankResults(
    ...
    try {
      ...
      this.logger.debug(
        `Ranking completed, top score: ${rankedResults[0]?.finalScore}`,
      );
  ...
  private calculateUrgencyScore(
    ...
    const hasUrgentQuery = urgentKeywords.some(
      (keyword) =>
  ...
  private calculatePreferenceScore(
    ...
    if (preferences.categoryWeights && result.dump.category?.name) {
      const categoryWeight =
        preferences.categoryWeights[result.dump.category.name] || 1;
    ...
    if (preferences.preferRecent) {
      const daysSinceCreation =
        (Date.now() - new Date(result.dump.created_at).getTime()) /
        (1000 * 60 * 60 * 24);
    ...
    if (
      preferences.preferHighUrgency &&
      (result.dump.urgency_level || 1) >= 3
    ) {
      score += 0.08;
    }
  ...
  private calculateCategoryScore(
    ...
    for (const queryCategory of query.entities.categories) {
      if (
        categoryName.includes(queryCategory.toLowerCase()) ||
  ...
  private calculateContentSimilarity(
    ...
    const content1 = (
      result1.dump.ai_summary ||
      result1.dump.raw_content ||
      ''
    ).toLowerCase();
    const content2 = (
      result2.dump.ai_summary ||
      result2.dump.raw_content ||
      ''
    ).toLowerCase();
  ...
  getRankingExplanation(
    ...
    if (context?.userPreferences) {
      components.splice(
        5,
        0,
        `Preferences: +${this.calculatePreferenceScore(result, context.userPreferences).toFixed(3)}`,
      );
    }
